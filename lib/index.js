...
class DiscordDatabase {
  ...
  async insertOne(content, channel = { name: "", id: "" }) {
    try {
      ...
      const res = await axios.post(Routes.channelMessages(channelId),
        { content },
        {
          headers: {
  ...
  async find(channel = { name: "", id: "" }) {
    try {
      ...
      const res = await axios.get(Routes.channelMessages(channelId), {
        headers: {
          authorization: this.token,
        },
      });

      const result = res.data.map(({ id, content, attachments, timestamp }) => {
  ...
  */
  ...
  async updateOne(messageId, content, channel = { name: "", id: "" }) {
    try {
      ...
      const res = await axios.patch(Routes.channelMessage(channelId, messageId),
        { content },
        {
          headers: {
            authorization: this.token,
  ...
  async deleteMany(channel = { name: "", id: "" }) {
    try {
      ...
      const getId = await this.find(channel);
      for (let i = 0; i < getId.length; i++) {
        const res = await axios.delete(Routes.channelMessage(channelId, getId[i].id), {
          headers: {
            authorization: this.token,
          },
        });
  ...
  async uploadFile(file, fileName = null, channel = { name: "", id: "" }) {
    try {
      ...
      const res = await axios.post(Routes.channelMessages(channelId), form, {
        headers: {
          ...form.getHeaders(),
          authorization: this.token,
        },
      ...
      res.data.attachments[0].timestamp = Date.parse(res.data.timestamp);

      const result = res.data.attachments[0];
  ...
  async uploadFileWithContent(
    ...
    try {
      ...
      res.data.attachments[0].id = res.data.id;
      res.data.attachments[0].content = res.data.content
      res.data.attachments[0].timestamp = Date.parse(res.data.timestamp)
  ...
  async deleteMessageById(messageId, channel = { name: "", id: "" }) {
    try {
      ...
      const res = await axios.delete(Routes.channelMessage(channelId, messageId), {
        headers: {
          authorization: this.token,
        },
      });
