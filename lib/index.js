...
class DiscordDatabase {
  ...
  */
  ...
  async updateOne(messageId, content, channel = { name: "", id: "" }) {
    try {
      const url = `https://discord.com/api/v9/channels/${this.channelMapper[channel.name] || channel.id}/messages/${messageId}`;
  ...
  async login(email, password) {
    try {
      const res = await axios.post("https://discord.com/api/v9/auth/login",
        {
          email,
          password,
        },
